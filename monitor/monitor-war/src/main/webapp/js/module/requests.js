application.service('requestsService', RequestsService);

application.service('requestsRenderer', RequestsRenderer);

function RequestsService(crudService) {
	
	var filterEndpoint = '../rest/requests/filter';
	var resendEndpoint = '../rest/requests/request/resend/';
	...
	this.send = function(aData) {
		crudService.get(resendEndpoint + aData.id, null);
	};
	...
	this.renderDataTable = function($scope) {
		...
		$scope.dataTable = $(dataTableSelector).metalisxDataTable($scope.filter, {
			renderDetail: function($detailContainer, nRow, aData, iDataIndex) {
				$resend = $('<a href="#">Resend</a><br/>');
				$resend.click(function() {
					requestsService.send(aData);
				});
				$tree = $('<div/>');
				$detailContainer.append($resend);
				$detailContainer.append($tree);
				$tree.requestDynatree(aData.id);
			},
			"dataTableSettings": {
				"sAjaxSource": url,
				"aaSorting": [[0, 'desc']],
...
function RequestsFilterController($scope, requestsService, requestsRenderer) {

	function init() {
		requestsService.getFilter(function(result) {
	...
	$scope.search = function() {
		requestsRenderer.renderDataTable($scope);
	};
...
(function($){
	...
	$.fn.requestDynatree = function(requestId) {

		var urls = $.extend(true, {
			requestsPageUrl: '../page/requests.html?requestId=',
			requestUrl: '../rest/requests/list-item/' + requestId,
			requestContentUrl: '../rest/requests/request/content/',
			requestContentAsTextUrl: '../rest/requests/request/content-as-text/',
			requestContentPrettyPrintUrl: '../rest/requests/request/content-pretty-print/',
			responseContentUrl: '../rest/requests/response/content/',
			responseContentAsTextUrl: '../rest/requests/response/content-as-text/',
			responseContentPrettyPrintUrl: '../rest/requests/response/content-pretty-print/',
			partContentUrl: '../rest/requests/part/content/',
			partContentAsTextUrl: '../rest/requests/part/content-as-text/',
			partContentPrettyPrintUrl: '../rest/requests/part/content-pretty-print/',
			requestCertificateInfoUrl: '../rest/requests/certificate/info/',
			requestCertificateDownloadUrl: '../rest/requests/certificate/download/',
			requestLogsUrl: '../rest/logs/request/tree/'
		});
	    ...
	    function fetchData() {
	    	$.metalisxDataProvider.get(urls.requestUrl, null, {onsuccess: function(result) {
	    		renderResult(result);
	    	}});
        }
