 ...
 * 
 * Setting blockId
 * If the setting blockId is not null, it's value is set as value
 * of the id attribute of the created DIV element. By default 
 * it is null.
 * 
 * Setting blockClass
 * If the setting blockClass is not null, it's value is set as value
 * of the class attribute of the created DIV element. By default
 * it is null. When it is not null, the setting blockStyle is
 * ignored. The value needs to be provide as a CSS class in the page.
 *
 * Setting blockStyle
 * If the setting blockStyle is not null, it's value is set as value
 * of the style attribute of the created DIV element. By default 
 * it contains the styles to block the page for user input. 
 * 
 * Below is an example on how to overrule the default style by 
 * providing a CSS class.
 * Give as value for the setting blockClass: block. And include 
 * the following CSS class in the HTML page, make sure the URL
	...
	ngcBlock.directive('ngcBlock', function(crudService) {
	    ...
	    return {
	        ...
	        link:function (scope, element, attrs) {
	        	...
	        	var keydownObject = null;

	        	function block() {
					if (!blockInProgress) {
						blockInProgress = true;
						// Catch all key strokes to do nothing
		        		keydownObject = $('body').on('keydown keypress', function() {
			        		return false;
			        	});
	        	...
	        	var blockEventOffFunction = scope.$on('ngc.block', function() {
	        		block();
	        	});
	
	        	var unblockEventOffFunction = scope.$on('ngc.unblock', function() {
	        		unblock();
	        	});
